...
let res = {};
if(this.data.userid) {
res = await page(`zhuguantitijiao`, searchForm);
} else {
res = await list(`zhuguantitijiao`, searchForm);
}
...
wx.navigateTo({
url: `/pages/zhuguantitijiao/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("zhuguantitijiao",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/zhuguantitijiao/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
url: `/pages/zhuguantitijiao/detail`
})
...
this.setData({
addAuth: utils.isAuth("zhuguantitijiao", "新增") || utils.isAuthFront("zhuguantitijiao", "新增"),
delAuth: utils.isAuth("zhuguantitijiao", "删除") || utils.isAuthFront("zhuguantitijiao", "删除"),
editAuth: utils.isAuth("zhuguantitijiao", "修改") || utils.isAuthFront("zhuguantitijiao", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("zhuguantitijiao",obj)
    }else{
         resList= await list("zhuguantitijiao",obj)
    }
