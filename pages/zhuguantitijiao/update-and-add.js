...
detailList,
    ...
    this.data.editStatus? getApp().globalData.editorContent=data.tijiaodaan:""
...
const obj={
    ...
    tijiaodaan: getApp().globalData.editorContent,
...
const tableName= `zhuguantitijiao`
...
let tijiaodaan=  getApp().globalData.editorContent
ruleForm['tijiaodaan']=getApp().globalData.editorContent
if(tijiaodaan==='' ){
wx.showToast({
    icon: "none",
    title: `提交答案不能为空`,
...
let corssRes = await list(`zhuguantitijiao`, params)
crossoptnum = wx.getStorageSync('statusColumnName');
...
if (ruleForm.id ) {
await update(`zhuguantitijiao`, ruleForm)
}
else {
await add(`zhuguantitijiao`, ruleForm)
}
}
