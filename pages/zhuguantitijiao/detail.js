...
zhuguantipingfenShow:'',

},
async    onLoad(option) {
this.setData({
    zhuguantipingfenShow: utils.isAuthFront('zhuguantitijiao','评分'),
})
...
tablename: 'zhuguantitijiao',
userid:userInfo.id,
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.shijuanmingcheng,
...
const {
data
} = await detail("zhuguantitijiao",id)
this.setData({
payAuth:utils.isAuthFront('zhuguantitijiao','支付')
})
...
const resUpdate = await update('zhuguantitijiao', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','zhuguantitijiao');
...
async reply() {
const detailList = this.data.detailList
const res = await update("zhuguantitijiao", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','zhuguantitijiao');
