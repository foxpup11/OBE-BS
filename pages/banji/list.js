...
let res = {};
if(this.data.userid) {
res = await page(`banji`, searchForm);
} else {
res = await list(`banji`, searchForm);
}
...
wx.navigateTo({
url: `/pages/banji/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("banji",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/banji/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("banji", "新增") || utils.isAuthFront("banji", "新增"),
delAuth: utils.isAuth("banji", "删除") || utils.isAuthFront("banji", "删除"),
editAuth: utils.isAuth("banji", "修改") || utils.isAuthFront("banji", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("banji",obj)
    }else{
         resList= await list("banji",obj)
    }
