...
const {
data
} = await detail("banji",id)
this.setData({
payAuth:utils.isAuthFront('banji','支付')
})
...
const resUpdate = await update('banji', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','banji');
...
async reply() {
const detailList = this.data.detailList
const res = await update("banji", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
