...
let res = {};
if(this.data.userid) {
res = await page(`zhuanye`, searchForm);
} else {
res = await list(`zhuanye`, searchForm);
}
...
wx.navigateTo({
url: `/pages/zhuanye/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("zhuanye",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/zhuanye/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("zhuanye", "新增") || utils.isAuthFront("zhuanye", "新增"),
delAuth: utils.isAuth("zhuanye", "删除") || utils.isAuthFront("zhuanye", "删除"),
editAuth: utils.isAuth("zhuanye", "修改") || utils.isAuthFront("zhuanye", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("zhuanye",obj)
    }else{
         resList= await list("zhuanye",obj)
    }
