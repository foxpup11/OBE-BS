...
const {
data
} = await detail("zhuanye",id)
this.setData({
payAuth:utils.isAuthFront('zhuanye','支付')
})
...
const resUpdate = await update('zhuanye', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','zhuanye');
...
async reply() {
const detailList = this.data.detailList
const res = await update("zhuanye", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
