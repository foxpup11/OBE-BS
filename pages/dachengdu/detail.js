...
const {
data
} = await detail("dachengdu",id)
this.setData({
payAuth:utils.isAuthFront('dachengdu','支付')
})
...
const resUpdate = await update('dachengdu', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','dachengdu');
...
async reply() {
const detailList = this.data.detailList
const res = await update("dachengdu", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
