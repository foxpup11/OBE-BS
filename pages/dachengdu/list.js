...
let res = {};
if(this.data.userid) {
res = await page(`dachengdu`, searchForm);
} else {
res = await list(`dachengdu`, searchForm);
}
...
wx.navigateTo({
url: `/pages/dachengdu/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("dachengdu",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/dachengdu/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("dachengdu", "新增") || utils.isAuthFront("dachengdu", "新增"),
delAuth: utils.isAuth("dachengdu", "删除") || utils.isAuthFront("dachengdu", "删除"),
editAuth: utils.isAuth("dachengdu", "修改") || utils.isAuthFront("dachengdu", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("dachengdu",obj)
    }else{
         resList= await list("dachengdu",obj)
    }
