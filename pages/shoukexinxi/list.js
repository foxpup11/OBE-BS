...
if(this.data.zhuanye){
    searchForm['zhuanye'] = '%' + this.data.zhuanye + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`shoukexinxi`, searchForm);
} else {
res = await list(`shoukexinxi`, searchForm);
}
...
wx.navigateTo({
url: `/pages/shoukexinxi/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("shoukexinxi",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/shoukexinxi/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("shoukexinxi", "新增") || utils.isAuthFront("shoukexinxi", "新增"),
delAuth: utils.isAuth("shoukexinxi", "删除") || utils.isAuthFront("shoukexinxi", "删除"),
editAuth: utils.isAuth("shoukexinxi", "修改") || utils.isAuthFront("shoukexinxi", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("shoukexinxi",obj)
    }else{
         resList= await list("shoukexinxi",obj)
    }
