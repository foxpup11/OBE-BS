...
let res = {};
if(this.data.userid) {
res = await page(`zhuguanti`, searchForm);
} else {
res = await list(`zhuguanti`, searchForm);
}
...
wx.navigateTo({
url: `/pages/zhuguanti/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("zhuguanti",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/zhuguanti/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("zhuguanti", "新增") || utils.isAuthFront("zhuguanti", "新增"),
delAuth: utils.isAuth("zhuguanti", "删除") || utils.isAuthFront("zhuguanti", "删除"),
editAuth: utils.isAuth("zhuguanti", "修改") || utils.isAuthFront("zhuguanti", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("zhuguanti",obj)
    }else{
         resList= await list("zhuguanti",obj)
    }
