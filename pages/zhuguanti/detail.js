...
async    onLoad(option) {
this.setData({
    zhuguantitijiaoShow: utils.isAuthFront('zhuguanti','提交'),
})
...
const {
data
} = await detail("zhuguanti",id)
this.setData({
payAuth:utils.isAuthFront('zhuguanti','支付')
})
...
const resUpdate = await update('zhuguanti', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','zhuguanti');
...
async reply() {
const detailList = this.data.detailList
const res = await update("zhuguanti", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
...
wx.setStorageSync('crossTable','zhuguanti');
