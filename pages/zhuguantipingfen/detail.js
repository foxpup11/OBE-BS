...
const {
data
} = await detail("zhuguantipingfen",id)
this.setData({
payAuth:utils.isAuthFront('zhuguantipingfen','支付')
})
...
const resUpdate = await update('zhuguantipingfen', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','zhuguantipingfen');
...
async reply() {
const detailList = this.data.detailList
const res = await update("zhuguantipingfen", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
