...
if(this.data.shijuanmingcheng){
    searchForm['shijuanmingcheng'] = '%' + this.data.shijuanmingcheng + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`zhuguantipingfen`, searchForm);
} else {
res = await list(`zhuguantipingfen`, searchForm);
}
...
let targetName="shijuanmingcheng"
...
wx.navigateTo({
url: `/pages/zhuguantipingfen/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("zhuguantipingfen",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/zhuguantipingfen/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
url: `/pages/zhuguantipingfen/detail`
})
...
this.setData({
addAuth: utils.isAuth("zhuguantipingfen", "新增") || utils.isAuthFront("zhuguantipingfen", "新增"),
delAuth: utils.isAuth("zhuguantipingfen", "删除") || utils.isAuthFront("zhuguantipingfen", "删除"),
editAuth: utils.isAuth("zhuguantipingfen", "修改") || utils.isAuthFront("zhuguantipingfen", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("zhuguantipingfen",obj)
    }else{
         resList= await list("zhuguantipingfen",obj)
    }
