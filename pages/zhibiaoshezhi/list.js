...
let res = {};
if(this.data.userid) {
res = await page(`zhibiaoshezhi`, searchForm);
} else {
res = await list(`zhibiaoshezhi`, searchForm);
}
...
wx.navigateTo({
url: `/pages/zhibiaoshezhi/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("zhibiaoshezhi",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/zhibiaoshezhi/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
wx.navigateTo({
url: `/pages/zhibiaoshezhi/detail`
})
...
this.setData({
addAuth: utils.isAuth("zhibiaoshezhi", "新增") || utils.isAuthFront("zhibiaoshezhi", "新增"),
delAuth: utils.isAuth("zhibiaoshezhi", "删除") || utils.isAuthFront("zhibiaoshezhi", "删除"),
editAuth: utils.isAuth("zhibiaoshezhi", "修改") || utils.isAuthFront("zhibiaoshezhi", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("zhibiaoshezhi",obj)
    }else{
         resList= await list("zhibiaoshezhi",obj)
    }
