...
if(this.data.zhishidian){
    searchForm['zhishidian'] = '%' + this.data.zhishidian + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`zhishidian`, searchForm);
} else {
res = await list(`zhishidian`, searchForm);
}
...
wx.navigateTo({
url: `/pages/zhishidian/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("zhishidian",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/zhishidian/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("zhishidian", "新增") || utils.isAuthFront("zhishidian", "新增"),
delAuth: utils.isAuth("zhishidian", "删除") || utils.isAuthFront("zhishidian", "删除"),
editAuth: utils.isAuth("zhishidian", "修改") || utils.isAuthFront("zhishidian", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("zhishidian",obj)
    }else{
         resList= await list("zhishidian",obj)
    }
