...
const {
data
} = await detail("zhishidian",id)
this.setData({
payAuth:utils.isAuthFront('zhishidian','支付')
})
...
const resUpdate = await update('zhishidian', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','zhishidian');
...
async reply() {
const detailList = this.data.detailList
const res = await update("zhishidian", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
