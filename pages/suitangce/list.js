...
let res = {};
if(this.data.userid) {
res = await page(`suitangce`, searchForm);
} else {
res = await list(`suitangce`, searchForm);
}
...
wx.navigateTo({
url: `/pages/suitangce/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("suitangce",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/suitangce/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("suitangce", "新增") || utils.isAuthFront("suitangce", "新增"),
delAuth: utils.isAuth("suitangce", "删除") || utils.isAuthFront("suitangce", "删除"),
editAuth: utils.isAuth("suitangce", "修改") || utils.isAuthFront("suitangce", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("suitangce",obj)
    }else{
         resList= await list("suitangce",obj)
    }
