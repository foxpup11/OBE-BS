...
this.setData({
isAuthReply:utils.isAuthFront("suitangce", "审核")
})
...
const {
data
} = await detail("suitangce",id)
this.setData({
payAuth:utils.isAuthFront('suitangce','支付')
})

this.setData({
goodname:data.xuehao
})
...
const resUpdate = await update('suitangce', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','suitangce');
...
const res = await update("suitangce", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
