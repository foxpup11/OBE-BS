...
ro:{
    ...
    touxiang:"",
        ...
        xingbieIndex:null,
    shoujihaoma:"",
        banjiList:"${column.customize}".split(','),
        banjiIndex:null,
},
...
const banjires = await option('banji/banji')
        banjires.data.unshift('请选择班级')
this.setData({
        banjiList: banjires.data
})
...
const  def_5= null  ;
    this.data.xingbieList.map((v, index) => {
        ...
        banjires.data.map((item, index) => {
        if (item == data.banji) {
            this.setData({
                    banjiIndex: index,
                    banji: item
            })
                                                                                                                                                                                                                        }else if (this.data.banjiList.length == 1) {
            this.setData({
                    banjiIndex: 0,
                    banji: v
            })
...
async banjiChange(e) {
const selectedIndex = e.detail.value;
let  banji=this.data.banjiList[selectedIndex]
this.setData({
...
if(this.data.banjiList[this.data.banjiIndex]==undefined ){
wx.showToast({
    icon: "none",
    title: `班级不能为空`,
})
