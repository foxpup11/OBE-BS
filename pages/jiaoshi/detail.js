...
const {
data
} = await detail("jiaoshi",id)
this.setData({
payAuth:utils.isAuthFront('jiaoshi','支付')
})
...
const resUpdate = await update('jiaoshi', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','jiaoshi');
...
async reply() {
const detailList = this.data.detailList
const res = await update("jiaoshi", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
