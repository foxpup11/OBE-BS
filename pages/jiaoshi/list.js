...
if(this.data.jiaoshixingming){
    searchForm['jiaoshixingming'] = '%' + this.data.jiaoshixingming + '%'
}


let res = {};
if(this.data.userid) {
res = await page(`jiaoshi`, searchForm);
} else {
res = await list(`jiaoshi`, searchForm);
}
...
wx.navigateTo({
url: `/pages/jiaoshi/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("jiaoshi",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/jiaoshi/update-and-add?id=${id}`
})
},
async detailBtn(e) {
...
this.setData({
addAuth: utils.isAuth("jiaoshi", "新增") || utils.isAuthFront("jiaoshi", "新增"),
delAuth: utils.isAuth("jiaoshi", "删除") || utils.isAuthFront("jiaoshi", "删除"),
editAuth: utils.isAuth("jiaoshi", "修改") || utils.isAuthFront("jiaoshi", "修改"),
})
...
obj['limit']=this.data.pageSize
    if(this.data.userid){
        resList = await page("jiaoshi",obj)
    }else{
         resList= await list("jiaoshi",obj)
    }
